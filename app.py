...
BARBEROS = {
    "1": {"nombre": "Juan Carlos", "telefono": "50685968072"},
    "2": {"nombre": "Jose Luis", "telefono": "50685968072"},
    "3": {"nombre": "William", "telefono": "50685968072"}
}
ALIAS_SERVICIOS = {
...
def enviar_whatsapp_template_cancelacion_cliente(numero, barbero, servicio, fecha, hora, cliente):
    try:
        if not WHATSAPP_TOKEN or not WHATSAPP_PHONE_NUMBER_ID:
            print("WhatsApp no configurado en variables de entorno.")
            return None
        ...
        payload = {
            ...
            "type": "template",
            "template": {
                "name": "cancelacion_barbero_cliente_cr",
                "language": {
                    "code": "es_CR"
                },
                "components": [
        ...
        print("Error enviando template de cancelación al cliente:", e)
        return None    
...
def agendar():
    try:
        ...
        if fecha == hoy_cr and not cumple_colchon_minimo(fecha, hora, MINUTOS_COLCHON):
          flash(f"Debes agendar con al menos {MINUTOS_COLCHON} minutos de anticipación.")
          return redirect(url_for("index"))
    ...
    while actual + timedelta(minutes=duracion_nueva) <= cierre:
        ...
        if libre:
            if fecha == fecha_hoy_cr:
                ...
                if hora_slot_hoy >= ahora_cr + timedelta(minutes=MINUTOS_COLCHON):
...
def cancelar_barbero():
    ...
    if cita:
        ...
        if cliente_telefono:
            r_cancelacion_cliente = enviar_whatsapp_template_cancelacion_cliente(
                numero=cliente_telefono,
                barbero=nombre_barbero,
                servicio=cita.get("servicio", ""),
                fecha=fecha,
            ...
            if r_cancelacion_cliente is None or r_cancelacion_cliente.status_code not in [200, 201]:
                print("No se pudo enviar la notificación de cancelación al cliente.")
